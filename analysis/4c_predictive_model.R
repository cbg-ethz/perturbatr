...
analyse <- function(md)
{
  ...
  lmm.fit     <- lme4::lmer(Readout ~ Virus + (1 | GeneSymbol) + (1 | Virus:GeneSymbol) + (1 | ScreenType) + (1 | Virus:ScreenType),
                            data = md, weights = md$Weight, verbose = F)
  fits <- list(lmm.fit=lmm.fit, pmm.fit=pmm.fit)
  fits
}
...
mse <- function(cv.test, fits)
{
  ...
  {
    pred <- predict(fit, data) %>% as.data.table
...
.cv.validate <- function(model.data)
{
  ...
  for (i in seq(max.cv))
  {
    ...
    li[[i]]  <- data.table(Sampling="CV", LMM_MSE=mses$lmm, PMM_MSE=mses$pmm)
...
run <- function()
{
  ...
  model.data  <- perturbatr::set.lmm.model.data(
