...
method. We use one of the provided data-sets for this.

```{r, eval=T, include=T}
  data(rnaiscreen)
```

Before we do any normalisations we should have a look at quality scores and single plates before:

```{r, eval=TRUE, include=TRUE, warning=FALSE, fig.align="center", fig.width=4}
  v1.data         <- knockout::filter(rnaiscreen, Virus == "V1")
...
```{r, eval=TRUE, include=T, fig.align="center", fig.width=4}
...
```{r, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
  v1.data.norm <- knockout::preprocess(v1.data, normalize=c("log", "robust-z.score"))
```
...
```{r, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
  v1.dat <- knockout::filter(rnaiscreen, Virus=="V1")
...
or a test using a <i>Chi-squared<i> distributed test statistic using `chisq.statistic`. 
We've also implemented prioritized detection of hits using a penalized linear model by [@schmich2015gesper]
using the `gespeR` function.
...
We describe the phenotype/readout by a linear combination of fixed effects and random effects (inspired by [@ramo2014simultaneous])
