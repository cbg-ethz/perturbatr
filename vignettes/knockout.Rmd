...
```{r, eval=T, include=T, fig.align="center", fig.width=6}
  ...
  hcv.genome.raw          <- filter(rnai.screen.raw, Virus=="HCV",  Screen=="Genome")
  hcv.kinome.raw.quality  <- quality(hcv.kinome.raw)
  hcv.genome.raw.quality  <- quality(hcv.genome.raw)
...
```{r, eval=TRUE, include=T, fig.align="center", fig.width=6}
  repls <- replicates(hcv.kinome.raw)
...
```{r, eval=TRUE, include=T, fig.align="center", fig.width=6}
  repls <- replicates(hcv.kinome.norm)
  plot(repls[1], repls[2])
```
