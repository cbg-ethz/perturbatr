...
```{r, eval=TRUE, include=TRUE, warning=FALSE, fig.align="center", fig.width=3}
...
```{r, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE, fig.width=3}
...
We also refer the interested reader to a recent model for hit prioritization using a penalized linear model by [@schmich2015gesper].
...
```{r eval=TRUE, include=TRUE, fig.align="center", fig.width=3, warning=FALSE, message=FALSE}
  pl <- plot(res)
  print(pl[[1]])
```
...
```{r eval=TRUE, include=TRUE, fig.align="center", fig.width=3, message=FALSE}
  graph.file <- system.file("extdata", "graph_file.tsv", package = "knockout")
  diffu <- diffuse(res, method="mrw", path=graph.file)
  plot(diffu)
```
