...
```{r, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
  ...
  res.hm <- hm(rnaiscreen, formula = frm, effect.size=0.01)
```

Let's visualize the results from this analysis:
...
After that we also do network diffusion using a genetic interaction network:

```{r, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
  diffuse(res.hm, path="graph_file.tsv")
```
