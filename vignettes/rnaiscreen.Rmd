...
Here we show an example data analysis using a pan-pathogenic data set of two RNAi screening studies.
The data set consists of two integrated kinome screens and have been 
...
For the sake of simplicity, you can model your readout as a linear combination
of fixed effects and clustering effects for genes and genes 
nested in a condition:

$$y_{cgp} = x_c \beta + \gamma_g  + \delta_{cg} + \dots + y_{cgp}$$

where $y_{cgpt}$ is the phenotype for a condition $c$, gene $g$ and 
perturbation $p$. This would correspond to the `formula`

```{r, eval=FALSE, include=TRUE, warning=FALSE, message=FALSE}
  Readout ~ Condition + (1|GeneSymbol) + (1|Condition:GeneSymbol)
```

which is the default argument. You can see that we model the gene effect using 
different intercepts for each gene. To use the hierarchical model with the 
above formula, call:
...
Depending on your data set different clustering effects might be present of 
course. So let's have a look at our data again:

```{r, include=TRUE, size="tiny"}
  dataSet(rnaiscreen)
```

For viral perturbation screens, as simulated by our example data set, we know 
...
for the stage of the infection. We estimated the effects using `lme4` 
[@bates2014lme4].
...
plots. The most informative one that shows the top 25 genes:
...
For that we need a graph file and call the `diffuse` function. If we plot the
results we get a list of reranked genes:
