...
less genes than the SARS and DENV data sets. That is no problem however, because we automatically 
filter such that the genes are same. We also automatically remove positive controls.

Next we rank the genes using a hierarchical model which requires describing the model to use as
R `formula`. The data has the following covariates:
...
and corrected for batch effects. However, the `Readout`s should be different between 
`ScreenType`s:
...
Plotting the `res.hm` objet yields a list of multiple
