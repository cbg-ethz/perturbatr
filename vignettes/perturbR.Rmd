...
output:
  BiocStyle::html_document:
    ...
    toc_depth: 2
    toc_float: true
bibliography: perturbR.bib
vignette: >
  %\VignetteIndexEntry{perturbR cookbook}
...
```{r, eval=TRUE, include=TRUE}
  perturbR::select(my.perturbation.data, GeneSymbol, Readout)
  rbind(perturbR::filter(my.perturbation.data, Readout > 0),
        perturbR::filter(my.perturbation.data, Readout < 0))
```
...
```{r, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE}
  v1.data      <- perturbR::filter(rnaiscreen, Condition == "V1")
  v1.data.norm <- perturbR::preprocess(v1.data)
```
...
```{r, eval=TRUE, include=T}
  v1.dat <- perturbR::filter(rnaiscreen, Condition=="V1")
  v2.dat <- perturbR::filter(rnaiscreen, Condition=="V2")
```
...
```{r, eval=TRUE, include=TRUE, warning=FALSE, message=FALSE, fig.align="center", fig.width=4}
  v1.dat <- perturbR::filter(rnaiscreen, Condition=="V1")
  v1.data.norm <- perturbR::preprocess(v1.dat)
