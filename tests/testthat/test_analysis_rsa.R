...
testthat::test_that("error is thrown when summarization is not logical", {
  testthat::expect_error(
    hyper.statistic(rnai.norm, do.summarization="no"))
})

testthat::test_that("error is thrown when summarization and leven is sirna", {
  testthat::expect_error(
    hyper.statistic(rnai.norm, do.summarization=TRUE, level="sirna"))
})
...
testthat::test_that("different levels give different results", {
  ...
  rows <- cols <- 1:20
  readouts     <- rnorm(20)
  h.sirna <- knockdown:::.hypertest(all.genes, all.sirnas,
                                    plates, rows, cols, readouts, "sirna")
  h.gene  <- knockdown:::.hypertest(all.genes, all.sirnas,
...
testthat::test_that("hypertest on group works", {
  ranks    <- c(1, 2, 3)
  all.genes <- letters
  v <- data.table(A=knockdown:::.hypertest.grp(ranks, all.genes)) %>%
