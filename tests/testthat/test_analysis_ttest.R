...
testthat::test_that("warning is thrown when only two replicates", {
  testthat::expect_warning(invisible(perturbation::.t.test("gene", c(1, 2), 0)))
})

testthat::test_that("p value is one for two replicates", {
  options(warn=0)
  testthat::expect_warning(
    testthat::expect_equal(perturbation:::.t.test("gene", c(1, 2), 0)$p.value, 1)
  )
})
...
testthat::test_that("hit ratio for prioritization for t test is working", {
  ...
  pri <- perturbation:::.prioritize.tstatistic(dat)
